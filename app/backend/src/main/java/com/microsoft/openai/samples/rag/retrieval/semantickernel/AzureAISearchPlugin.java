...
/**
 * This class is Semantic Kernel plugin that provides a kernel semantic function to search
 * information relevant to answering a given query. It uses Azure AI Search to retrieve information
 * from the search index. The implementation simply delegates to the common AzureAISearchRetriever
 * class which is also used by the RAG approach based on java plain open ai client sdk.
 */
public class AzureAISearchPlugin {
    ...
    public AzureAISearchPlugin(
            AzureAISearchProxy azureAISearchProxy, OpenAIProxy openAIProxy, RAGOptions options) {
    ...
    public Mono<List<ContentSource>> searchFromQuestion(
            @KernelFunctionParameter(description = "the query to answer", name = "query")
                    String query) {
    ...
    public Mono<List<ContentSource>> searchFromConversation(
        ...
        // Parse conversation
        List<ChatGPTMessage> chatMessages =
        ...
        List<ContentSource> sources =
                retriever.retrieveFromConversation(
                        new ChatGPTConversation(chatMessages), this.options);

        LOGGER.info("Total {} sources found in Azure AI search", sources.size());
